...
int start_server(const std::string& ip_address, const int& port)
{
    ...
    const std::string message = "Arc is listening on " + ip_address + ":" + std::to_string(port);
    ...
    while (return_code == 1)
    {
        ...
        if (!strings::is_empty(input_string))
        {
            ...
            {
				arc::print_prompt();
				std::vector<std::string> tokens = styl.read(body);
				return_code = arc::exec_tokens(tokens);
                return_code = return_code == -1 ? 0 : return_code;
